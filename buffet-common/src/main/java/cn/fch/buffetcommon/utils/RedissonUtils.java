...
public class RedissonUtils {
    ...
    static {
        ...
        //2.设置Redis服务器的地址和密码
        config.useSingleServer().setAddress("redis://" + ConstantConfig.REDIS_HOST);
        //3.实例化客户端对象
    ...
    public static void setStr(String key, Object val, long seconds) {
        client.getBucket(key).set(val, seconds == 0 ? ConstantConfig.DEFAULT_EXPIRED : seconds, TimeUnit.MILLISECONDS);
    }
    ...
    public static void expire(String key, long seconds) {
        client.getKeys().expire(key, seconds == 0 ? ConstantConfig.DEFAULT_EXPIRED : seconds, TimeUnit.MILLISECONDS);
    }
